    ...
    <div className='container mx-auto min-h-dvh space-y-5'>
      <div>
        ...
        <div className='grid grid-cols-3 bg-base-200 p-3'>
          <div className='flex flex-col'>
            ...
            <h2 className='text-primary text-3xl'>{tab === "plan" ? todaysPlan.length : saved.length}</h2>
          </div>
          <div className='flex flex-col'>
      ...
      <div className='flex flex-row justify-between items-center'>
        <div className='space-x-3 bg-base-200 rounded-2xl p-1'>
          <button onClick={() => setTab("plan")} className={`${tab === "plan" ? "text-primary bg-black rounded-l-2xl" : ""} py-2 px-4`}>Todays Plan</button>
          <button onClick={() => setTab("saved")} className={`${tab === "saved" ? "text-primary bg-black rounded-r-2xl" : ""} py-2 px-4`}>Saved</button>
        </div>
        <div>
          <p>Sort by</p>
          <select>
      ...
      <div className='flex flex-col gap-3'>
        {
          ...
          ? todaysPlan.length===0
            ?<div className='flex flex-col justify-center items-center mt-20 space-y-2'>
              ...
              <Link href={'/'} className='bg-primary py-2 px-7 mt-5 rounded-2xl text-black'>Go to workouts</Link>
            </div>
